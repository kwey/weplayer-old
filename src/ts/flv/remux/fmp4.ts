...
class FMP4 {
    ...
    static dinf() {
        ...
        buffer.write(FMP4.size(36), fmp4type('dinf'), FMP4.size(28), fmp4type('dref'), new Uint8Array(dref))
        return buffer.buffer
    }
    ...
    static esds(config = [43, 146, 8, 0]) {
        ...
        buffer.write(FMP4.size(8 + content.byteLength), fmp4type('esds'), content)
        return buffer.buffer
    }
    static avc1(data: any) {
        ...
        buffer.write(
            FMP4.size(size + avc1.byteLength + avcc.byteLength + btrt.byteLength), fmp4type('avc1'), avc1,
            FMP4.size(8 + avcc.byteLength), fmp4type('avcC'), avcc,
            FMP4.size(20), fmp4type('btrt'), btrt,
            FMP4.size(16), fmp4type('pasp'), pasp
        )
        return buffer.buffer
    }
    static stts() {
    ...
    static mvex(duration: any) {
        ...
        buffer.write(FMP4.size(88), fmp4type('mvex'), FMP4.size(16), fmp4type('mehd'), FMP4.extension(0, 0), mehd, FMP4.trex(1), FMP4.trex(2))
        return buffer.buffer
    }
    static trex(id: any) {
        const content = new Uint8Array([
    ...
    static trun(data: any, sdtpLength: any) {
        ...
        // data-offset 4
        // samples.length
        const offset = Buffer.writeUint32(8 + 8 + 16 + 8 + 16 + 16 + 12 + 4 + 4 + 16 * data.samples.length + sdtpLength)
        buffer.write(FMP4.size(20 + 16 * data.samples.length), fmp4type('trun'), new Uint8Array([0x00, 0x00, 0x0F, 0x01]), sampleCount, offset)
    ...
    static sdtp(data: any) {
        const buffer = new Buffer()
        buffer.write(FMP4.size(12 + data.samples.length), fmp4type('sdtp'), FMP4.extension(0, 0))
        data.samples.forEach((item: any) => {
