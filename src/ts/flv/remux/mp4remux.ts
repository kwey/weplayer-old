...
import Remuxer from '../muxer'
import Store from '../models/store'

export default class Mp4Remuxer extends Remuxer {
    _dtsBase: any
    _isDtsBaseInited: any
    _videoMeta: any
    _audioMeta: any
    _audioNextDts: any
    _videoNextDts: any
    _videoSegmentList: any
    _audioSegmentList: any
    _fillSilenceFrame: any
    mediaFragment: Function
    _dtsBaseInited: boolean

    constructor(store: Store) {
        super(store, 'remuxer_')
        this._dtsBase = 0
    ...
    onMetaDataReady(type: string, meta: any) {
        this[`_${type}Meta`] = meta
    }

    onMediaInfoReady(mediaInfo: any) {
        const ftypMoov = new Buffer()
        const ftyp = FMP4.ftyp()
        const moov = FMP4.moov(mediaInfo)
        ftypMoov.write(ftyp, moov)
        return ftypMoov.buffer
    }

    calcDtsBase(audioTrack: any, videoTrack: any) {
    ...
    initSilentAudio(dts: any, duration: any) {
