...
export default class VideoDemuxer extends Demuxer {
        ...
        switch (packageType) {
            case 0: {
                ...
                this.store.metaEndPosition =
                    position + Buffer.readAsInt(tagSize) + 4 // 缓存scriptTag结束的位置，replay使用
    ...
    _parseAVCDecoderConfigurationRecord() {
        ...
        const naluLengthSize = (store.state.naluLengthSize =
            dv.getUint(2, this.readOffset, false) + 1)
        ...
        for (let i = 0; i < spsLength; i++) {
            ...
            if (mediaInfo.hasAudio) {
                if (mediaInfo.audioCodec) {
                    mediaInfo.mimeType = `video/x-flv codecs="${
