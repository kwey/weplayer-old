...
export default class AudioDemuxer extends Demuxer {
    ...
    resolve(tag: any) {
        ...
        if (soundFormatIdx !== 10 && soundFormatIdx !== 2) {
            ...
            if (aacInfo.packetType === 0) {
                ...
                if (mi.isComplete) {
                    this.mediaInfoReady(mi)
                }
            } else if (aacInfo.packetType === 1) {
                // AAC raw frame data
                const dts =
                    store.state.timeStampBase + this.currentTag.getTime()
                const aacSample = {
                    unit: aacInfo.data,
    ...
    _parseAACAudio(): any {
        ...
        if (!packetType) {
            ...
            this.store.metaEndPosition =
                position + Buffer.readAsInt(tagSize) + 4
    ...
    _parseAACAudioSpecificConfig() {
        ...
        const samplingIdx = ((UInt0 & getAndNum(5, 7)) << 1) | (UInt1 >>> 7) // UInt4
        if (samplingIdx < 0 || samplingIdx > samplingFrequencyTypes.length) {
            ...
            this.dispatch(
                EventTypes.ERROR,
                `error samplingFrequencyIndex: ${samplingIdx}`
            )
            return
        }
        ...
        const channelCount = (resultObj.channelCount =
            (UInt1 & getAndNum(1, 4)) >>> 3)
        if (channelCount < 0 || channelCount > 7) {
            ...
            this.dispatch(
                EventTypes.ERROR,
                `error Audio Channel Count: ${channelCount}`
            )
            return
        }
        ...
        if (audioObjectType === 5) {
            ...
            resultObj.extAudioSamplingIdx =
                ((UInt1 & getAndNum(5, 7)) << 1) | (UInt2 >>> 7)
    ...
    initAudioMeta(meta: any) {
        const { state, audioTrack: track } = this.store
