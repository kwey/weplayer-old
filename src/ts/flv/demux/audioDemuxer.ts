...
export default class AudioDemuxer extends Demuxer {
  ...
  resolve (tag: any) {
    ...
    if (soundFormatIdx !== 10 && soundFormatIdx !== 2) {
      ...
      if (aacInfo.packetType === 0) { // AAC sequence header
        ...
        const dts = store.state.timeStampBase + this.currentTag.getTime()
        const aacSample = { unit: aacInfo.data, length: aacInfo.data.byteLength, dts, pts: dts }
  ...
  _parseAACAudio (): any {
    ...
    const aacData: any = {}
    const aacArray = new Uint8Array(this.data.buffer, this.readOffset, this.unreadLength)
    const packetType = aacArray[0]
    this.readOffset += 1
    aacData.packetType = packetType
    if (!packetType) {
      const { position, tagSize } = this.currentTag
      this.store.metaEndPosition = position + Buffer.readAsInt(tagSize) + 4
  ...
  _parseAACAudioSpecificConfig () {
    ...
    const channelCount = resultObj.channelCount = (UInt1 & getAndNum(1, 4)) >>> 3
